            ...
            <div class="game-container">
                <div class="question-header">
                    <div class="flex justify-between items-center mb-4">
                        <div>
                            ...
                            <div class="mt-2 flex items-center gap-3">
                                <span class="level-badge">Level <span id="current-level">1</span> of <span
                                        id="total-levels">{{ ceil($questions->count() / 5) }}</span></span>
                    ...
                    <div class="flex justify-between text-sm mb-2">
                        <span>Question <span id="current-q">1</span> of <span
                                id="total-q">{{ $questions->count() }}</span></span>
                ...
                <div class="p-8">
                    ...
                    <div class="flex justify-between mt-8">
                        <button id="prev-question"
                            class="px-6 py-2 bg-gray-300 text-gray-800 rounded-lg hover:bg-gray-400 font-medium text-sm disabled:opacity-50 disabled:cursor-not-allowed">
                            <i class="fas fa-arrow-left mr-2"></i> Back
                        </button>
                        <button id="next-question"
                            class="next-btn px-6 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 font-medium text-sm disabled:opacity-50 disabled:cursor-not-allowed">
                    ...
                    <div id="level-complete-modal"
                        ...
                        <div class="bg-white rounded-2xl p-8 max-w-md text-center">
                            ...
                            <p class="text-gray-600 mb-4">Great job! You've completed <span
                                    id="completed-level-num">1</span> of <span
                                    id="total-levels-modal">{{ ceil($questions->count() / 5) }}</span> levels.</p>
    ...
    <script>
        ...
        function loadQuestion() {
            ...
            if (currentQuestion === 0) {
                ...
                prevBtn.classList.add('bg-white', 'text-white', 'cursor-not-allowed', 'shadow-none');
                prevBtn.classList.remove('bg-gray-300', 'text-gray-800', 'hover:bg-gray-400');
            } else {
                prevBtn.disabled = false;
                prevBtn.classList.remove('bg-white', 'text-white', 'cursor-not-allowed', 'shadow-none');
                prevBtn.classList.add('bg-gray-300', 'text-gray-800', 'hover:bg-gray-400');
            }
            ...
            if (answers[currentQuestion] !== null && answers[currentQuestion] !== undefined && answers[
                    currentQuestion] !==
                '') {
                const selectedDiv = document.querySelector(
                    `.option-item[data-value="${answers[currentQuestion].replace(/'/g, "\\'").replace(/"/g, '&quot;')}"]`
                );
                if (selectedDiv) {
        ...
        function handleFlagError(img, countryCode) {
            ...
            if (currentUrl.includes('flagpedia.net')) {
                ...
                img.onerror = function() {
                    ...
                    errorDiv.innerHTML =
                        `<i class="fas fa-flag text-gray-400 text-4xl mb-2 block"></i>Flag image not available<br><small class="text-xs">Country code: ${countryCode}</small>`;
