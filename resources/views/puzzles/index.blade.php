...
@section('content')
    ...
    <div class="min-h-screen bg-gray-50 py-8">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                        ...
                        <div>
                            ...
                            <select name="difficulty"
                                ...
                                @foreach ($difficulties as $key => $difficulty)
                                    <option value="{{ $key }}"
                                        {{ request('difficulty') == $key ? 'selected' : '' }}>
                                        {{ $difficulty }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select name="membership"
                                ...
                                <option value="free" {{ request('membership') == 'free' ? 'selected' : '' }}>Free
                                </option>
                                <option value="member" {{ request('membership') == 'member' ? 'selected' : '' }}>Members
                                    Only</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="flex justify-between items-center mt-4">
                        ...
                        <div class="flex space-x-3">
                            <select name="sort"
                                ...
                                <option value="difficulty_asc" {{ request('sort') == 'difficulty_asc' ? 'selected' : '' }}>
                                    Difficulty (Low to High)</option>
                                <option value="difficulty_desc"
                                    {{ request('sort') == 'difficulty_desc' ? 'selected' : '' }}>Difficulty (High to Low)
                                </option>
                            </select>
            ...
            <!-- Results Count -->
            <div class="mb-4">
                <p class="text-sm text-gray-600">Showing {{ $paginated->firstItem() ?? 0 }} -
                    {{ $paginated->lastItem() ?? 0 }} of {{ $paginated->total() }} puzzles</p>
            </div>
                        ...
                        <div class="puzzle-card">
                            @if ($puzzle->featured_image)
                                <div class="image-container">
                                    <img src="{{ $puzzle->featured_image }}" class="card-image"
                                        alt="{{ $puzzle->title }}">
                            ...
                            <div class="card-content">
                                ...
                                @else bg-red-100 text-red-800 @endif">
                                        ...
                                        @if ($puzzle->is_featured)
                                            <span
                                                class="ml-2 text-xs bg-yellow-100 text-yellow-800 px-2 py-1 rounded-full">Featured</span>
                                        @endif
                                    </div>
                                    @if ($puzzle->requires_membership)
                                        <span class="text-xs bg-purple-100 text-purple-800 px-2 py-1 rounded-full">Member
                                            Only</span>
                                    @endif
                                </div>
                                ...
                                <p class="text-sm text-gray-600 mb-4">
                                    {{ Str::limit($puzzle->short_description ?? $puzzle->description, 100) }}</p>

                                <div class="flex items-center justify-between text-sm text-gray-500 mb-4">
                                    @if (isset($puzzle->puzzle_type) && $puzzle->puzzle_type == 'wordsearch')
                                        <span><i class="fas fa-search mr-1"></i> {{ $puzzle->questions_count }}
                                            words</span>
                                    @elseif($puzzle->type == 'flag_match')
                                        <span><i class="fas fa-flag mr-1"></i> {{ $puzzle->questions_count ?? 0 }}
                                            flags</span>
                                    @else
                                        <span><i class="far fa-question-circle mr-1"></i>
                                            {{ $puzzle->questions_count ?? 0 }} questions</span>
                                    @endif
                                    <span><i class="fas fa-star mr-1 text-yellow-400"></i>
                                        {{ number_format($puzzle->average_rating, 1) }}
                                        ({{ $puzzle->total_ratings }})</span>
                                ...
                                <div class="flex space-x-3">
                                    @if (isset($puzzle->puzzle_type) && $puzzle->puzzle_type == 'wordsearch')
                                        ...
                                        @endif
                                    @elseif($puzzle->type == 'quiz' || $puzzle->type == 'capital_quiz' || $puzzle->type == 'flag_match' || $puzzle->type == 'country_puzzle')
