...
class PuzzleController extends Controller
{
    ...
    public function hub()
    {
        ...
        // Get categories with puzzle counts
        ...
        if ($donor) {
            $userStats = [
                ...
                'achievements' => DB::table('donor_achievements')
                    ->where('donor_id', $donor->id)
                    ->count(),
        ...
        // Global leaderboard (top 10 overall)
    ...
    public function index(Request $request)
    {
        ...
        $query = Puzzle::with('category')->where('is_active', true);
        ...
        if ($request->filled('membership') && $request->membership === 'member') {
            $query->where('requires_membership', true);
        } elseif ($request->filled('membership') && $request->membership === 'free') {
            $query->where('requires_membership', false);
        }
        ...
        $types = $this->puzzleTypes;
        $difficulties = $this->difficultyLevels;
        
        return view('puzzles.index', compact(
            'puzzles',
            'categories',
            'types',
            'difficulties',
   ...
   */
   public function globalLeaderboard()
   {
    ...
    public function show($slug)
    {
        ...
        if ($puzzle->requires_membership && !$member) {
            return redirect()->route('puzzles.index')
                ->with('error', 'This puzzle is for members only. Please become a member to access it.');
        }
        ...
        // Increment view count (but not on every refresh)
    ...
    public function start($slug)
    {
        ...
        if (!$donor) {
            return redirect()->route('donor.login')
                ->with('error', 'Please log in to play puzzles.');
        }
        
        $puzzle = Puzzle::where('slug', $slug)
            ->where('is_active', true)
            ->firstOrFail();
        ...
        if ($puzzle->requires_membership && !$member) {
            return redirect()->route('puzzles.show', $puzzle->slug)
                ->with('error', 'This puzzle is for members only. Please become a member to play.');
        }
        
        $attemptCount = PuzzleAttempt::where('donor_id', $donor->id)
            ->where('puzzle_id', $puzzle->id)
            ->count();
        
        if ($attemptCount >= $puzzle->attempts_allowed) {
            return redirect()->route('puzzles.show', $puzzle->slug)
        ...
        $attempt = PuzzleAttempt::create([
            ...
            'metadata' => [
                ...
                'screen_size' => request()->cookie('screen_size') ?? null,
            ],
        ]);
        
        Log::info('Puzzle attempt started', [
            'donor_id' => $donor->id,
            'puzzle_id' => $puzzle->id,
    ...
    public function play($attemptId)
    {
        ...
        // Load questions
        $questions = $puzzle->loadQuestions();
        ...
        if ($puzzle->is_timed && $puzzle->time_limit) {
            ...
            if ($timeElapsed > $puzzle->time_limit) {
                ...
                return redirect()->route('puzzles.results', ['attempt' => $attempt->id])
        ...
        $gameData = [
            'attempt' => [
                ...
                'hints_allowed' => $puzzle->hints_allowed,
            ],
            'puzzle' => [
                'id' => $puzzle->id,
                'title' => $puzzle->title,
                'type' => $puzzle->type,
                'type_icon' => $puzzle->type_icon,
        ...
        return view('puzzles.play', compact('puzzle', 'attempt', 'questions', 'gameData'));
    }
    ...
    public function submit(Request $request, $attemptId)
    {
        ...
        return response()->json([
            ...
            'feedback' => $result['feedback'],
            'redirect' => route('puzzles.results', ['attempt' => $attempt->id]),
        ]);
    }
    ...
    {
        $questions = $puzzle->questions()->where('is_active', true)->get();
        $score = 0;
        $feedback = [];
        ...
        $bonus = 0;
        if ($score == $questions->sum('points')) {
            $bonus = $puzzle->bonus_points;
            $score += $bonus;
    ...
    public function results($attemptId)
    {
        ...
        // Get achievements earned in this attempt
        $achievementsEarned = [];
        if ($donor) {
        ...
        return view('puzzles.results', compact(
            'attempt',
            'puzzle',
            'leaderboardEntry',
            'achievementsEarned',
    ...
    public function rate(Request $request, $slug)
    {
        ...
        $rating = PuzzleRating::updateOrCreate(
            ...
            [
                ...
                'feedback' => [
                    'difficulty' => $request->difficulty_rating,
                    'enjoyment' => $request->enjoyment_rating,
                ],
            ]
        );
        ...
        return response()->json([
            'success' => true,
            'rating' => $rating,
            'new_average' => $puzzle->average_rating,
        ]);
    }
    ...
    public function comment(Request $request, $slug)
    {
        ...
        if (!$donor) {
            return redirect()->route('donor.login')
                ->with('error', 'Please log in to comment.');
        }
    ...
    public function achievements()
    {
        ...
        if ($donor) {
            $earnedIds = DB::table('donor_achievements')
                ->where('donor_id', $donor->id)
